...
def number_info():
    ...
    try:
        ...
        # Check if all data fields (excluding author and number) are N/A
        check_data = {k: v for k, v in data.items() if k not in ["API Author", "Number"]}
        if all(v == "N/A" for v in check_data.values()):
            return jsonify({"error": "No data found for this number.", "API Author": "@codderpetro"}), 404

        return jsonify(data)

    except Exception as e:
        return jsonify({"error": str(e), "API Author": "@codderpetro"}), 500
